...
RSpec.describe CfdpMib do
  ...
  describe 'set_entity_value' do
    ...
    it 'sets cmd_info values' do
      CfdpMib.set_entity_value(1, 'cmd_info', %w[TARGET PACKET ITEM])
      expect(CfdpMib.entity(1)['cmd_info']).to eq(%w[TARGET PACKET ITEM])
    end

    it 'adds tlm_info values' do
      CfdpMib.set_entity_value(1, 'tlm_info', %w[TARGET PACKET ITEM])
      expect(CfdpMib.entity(1)['tlm_info']).to eq([%w[TARGET PACKET ITEM]])
    end
    ...
    it 'raises an error for invalid tlm_info' do
      expect do
        CfdpMib.set_entity_value(1, 'tlm_info', %w[TARGET PACKET])
      end.to raise_error(RuntimeError, /Invalid tlm_info/)
    end

    it 'raises an error for invalid cmd_info' do
      expect do
        CfdpMib.set_entity_value(1, 'cmd_info', %w[TARGET PACKET])
      end.to raise_error(RuntimeError, /Invalid cmd_info/)
    end
  end
  ...
  describe 'list_directory_files' do
    ...
    context 'when using S3 bucket' do
      ...
      it 'yields filenames for objects in S3 directory' do
        ...
        expect(filenames).to contain_exactly('/tmp/test_dir/file1.txt', '/tmp/test_dir/file2.txt')
      end
      ...
      it 'strips root path from S3 object keys' do
        ...
        expect(filenames).to contain_exactly('/tmp/test_dir/file1.txt', '/tmp/another_dir/file2.txt')
      end
  ...
  describe 'filestore_request' do
    ...
    it 'handles DELETE_FILE action when file exists' do
      status, = CfdpMib.filestore_request('DELETE_FILE', 'test_delete.txt', nil)
      expect(status).to eq('SUCCESSFUL')
    end

    it "handles DELETE_FILE action when file doesn't exist" do
      allow(File).to receive(:exist?).and_return(false)
      status, = CfdpMib.filestore_request('DELETE_FILE', 'missing.txt', nil)
    ...
    it 'handles RENAME_FILE action' do
      ...
      status, = CfdpMib.filestore_request('RENAME_FILE', 'old.txt', 'new.txt')
      expect(status).to eq('SUCCESSFUL')
    end
    ...
    it 'handles APPEND_FILE action' do
      status, = CfdpMib.filestore_request('APPEND_FILE', 'file1.txt', 'file2.txt')
      expect(status).to eq('SUCCESSFUL')
    end

    it 'handles REPLACE_FILE action' do
      status, = CfdpMib.filestore_request('REPLACE_FILE', 'file1.txt', 'file2.txt')
      expect(status).to eq('SUCCESSFUL')
    end

    it 'handles CREATE_DIRECTORY action' do
      status, = CfdpMib.filestore_request('CREATE_DIRECTORY', 'new_dir', nil)
      expect(status).to eq('SUCCESSFUL')
    end

    it 'handles REMOVE_DIRECTORY action' do
      status, = CfdpMib.filestore_request('REMOVE_DIRECTORY', 'test_dir', nil)
      expect(status).to eq('SUCCESSFUL')
    end

    it 'handles DENY_FILE action' do
      status, = CfdpMib.filestore_request('DENY_FILE', 'test_file.txt', nil)
      expect(status).to eq('SUCCESSFUL')
    end

    it 'handles DENY_DIRECTORY action' do
      status, = CfdpMib.filestore_request('DENY_DIRECTORY', 'test_dir', nil)
      expect(status).to eq('SUCCESSFUL')
    end
  ...
  describe 'setup' do
    ...
    it 'initializes MIB from options' do
      ...
      expect(CfdpMib.entity(2)['cmd_info']).to eq(%w[TARGET PACKET ITEM])
      expect(CfdpMib.entity(2)['tlm_info']).to include(%w[TARGET TLM_PKT ITEM])
  ...
  describe 'cleanup_old_transactions' do
    ...
    it 'logs the number of cleaned up transactions' do
      expect(OpenC3::Logger).to receive(:info).with('CFDP cleaned up 1 completed transactions',
    ...
    it 'handles multiple old transactions' do
      ...
      expect(OpenC3::Logger).to receive(:info).with('CFDP cleaned up 2 completed transactions',
                                                    scope: ENV['OPENC3_SCOPE'])
  ...
  describe 'load_saved_transactions' do
    ...
    it 'loads a source transaction successfully' do
      ...
      expect(CfdpSourceTransaction).to receive(:new).with(source_entity: CfdpMib.entity(1),
                                                          transaction_seq_num: 123) do |_args|
    ...
    it 'loads a receive transaction successfully' do
      ...
      expect(CfdpReceiveTransaction).to receive(:new).with(expected_pdu, no_persist: true) do |_args|
        # Simulate the constructor behavior of adding to transactions hash
    ...
    it 'handles failed state loading gracefully' do
      ...
      expect(CfdpSourceTransaction).to receive(:new).with(source_entity: CfdpMib.entity(1),
                                                          transaction_seq_num: 789).and_return(mock_transaction)

      CfdpMib.load_saved_transactions

      expect(CfdpMib.transactions).not_to have_key('1__789')
      expect(OpenC3::Logger).to have_received(:warn).with('CFDP failed to load saved transaction: 1__789',
                                                          scope: 'DEFAULT')
      expect(OpenC3::Logger).to have_received(:info).with('CFDP loaded 0 saved transactions', scope: 'DEFAULT')
    end
    ...
    it 'handles transaction creation errors and cleans up invalid state' do
      ...
      expect(OpenC3::Logger).to have_received(:error).with(
        'CFDP error loading saved transaction 1__999: Transaction creation failed', scope: 'DEFAULT'
      )
    ...
    it 'loads multiple transactions of different types' do
      saved_ids = %w[1__100 2__200 1__300]
      expect(CfdpTransaction).to receive(:get_saved_transaction_ids).and_return(saved_ids)
      ...
      expect(CfdpSourceTransaction).to receive(:new).with(source_entity: CfdpMib.entity(1),
                                                          transaction_seq_num: 100) do |_args|
        # Simulate the constructor behavior of adding to transactions hash
        CfdpMib.transactions['1__100'] = source_tx1
        source_tx1
      end
      expect(CfdpSourceTransaction).to receive(:new).with(source_entity: CfdpMib.entity(1),
                                                          transaction_seq_num: 300) do |_args|
        # Simulate the constructor behavior of adding to transactions hash
