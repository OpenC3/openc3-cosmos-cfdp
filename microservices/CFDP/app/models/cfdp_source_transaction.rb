...
class CfdpSourceTransaction < CfdpTransaction
  ...
  def put(
    ...
    put_options_data = {
      ...
      'messages_to_user' => JSON.generate(messages_to_user&.as_json, allow_nan: true),
      'filestore_requests' => JSON.generate(filestore_requests&.as_json allow_nan: true)
    }
  ...
  def copy_file_send_eof_pdu(
    ...
    CfdpTopic.write_indication("EOF-Sent", transaction_id: @id) if @source_entity['eof_sent_indication']
  ...
  def copy_file
    ...
    return unless put_options && !put_options.empty?

    destination_entity_id = put_options['destination_entity_id']&.to_i
    fault_handler_overrides = put_options['fault_handler_overrides'] ? JSON.parse(put_options['fault_handler_overrides']) : []
    ...
    messages_to_user = put_options['messages_to_user'] ? JSON.parse(put_options['messages_to_user']) : []
    filestore_requests = put_options['filestore_requests'] ? JSON.parse(put_options['filestore_requests']) : []
  ...
  def notice_of_completion
    ...
    if @source_entity['transaction_finished_indication']
  ...
  def save_state
    ...
    state_data = {
      ...
      'finished_pdu_hash' => JSON.generate(@finished_pdu_hash&.as_json, allow_nan: true),
      'destination_entity' => JSON.generate(@destination_entity&.as_json, allow_nan: true),
      'eof_count' => @eof_count,
      ...
      'eof_ack_pdu_hash' => JSON.generate(@eof_ack_pdu_hash&.as_json, allow_nan: true),
      'keep_alive_pdu_hash' => JSON.generate(@keep_alive_pdu_hash&.as_json, allow_nan: true),
      'copy_state' => @copy_state,
      'file_offset' => @file_offset,
      'file_checksum' => JSON.generate(@file_checksum&.as_json, allow_nan: true),
      'file_size' => @file_size,
      'read_size' => @read_size
    }
  ...
  def load_state(transaction_id)
    return false unless super(transaction_id, no_log: true)

    state_data = OpenC3::Store.hgetall("#{self.class.redis_key_prefix}cfdp_transaction_state:#{transaction_id}")

    @source_entity = state_data['source_entity_id'] ? CfdpMib.entity(state_data['source_entity_id']) : nil
    ...
    @file_offset = state_data['file_offset']&.to_i || 0
    @file_checksum = state_data['file_checksum'] ? JSON.parse(state_data['file_checksum']) : nil
    @file_size = state_data['file_size']&.to_i || 0
    @read_size = state_data['read_size']&.to_i || 0
