...
class CfdpReceiveTransaction < CfdpTransaction
  ...
  def save_state
    ...
    state_data = {
      ...
      'segments' => JSON.generate(@segments&.as_json, allow_nan: true),
      'eof_pdu_hash' => JSON.generate(@eof_pdu_hash&.as_json, allow_nan: true),
      'checksum_type' => @checksum.class.name,
      'full_checksum_needed' => @full_checksum_needed,
      'file_size' => @file_size,
      'filestore_responses' => JSON.generate(@filestore_responses&.as_json, allow_nan: true),
      ...
      'finished_ack_pdu_hash' => JSON.generate(@finished_ack_pdu_hash&.as_json, allow_nan: true),
      'prompt_pdu_hash' => JSON.generate(@prompt_pdu_has&.as_json allow_nan: true)
    }
